...
const StartGame: React.FC<StartGameProps> = ({ startGame }) => {
  ...
  const fetchPlayerGame = async () => {
    try {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_PORT}players`)
            ...
            <CardContent className="px-0 max-h-[250px] md:max-h-[650px] overflow-y-auto scroll-hide">
              {playerState.length > 0 && playerState.map((player: Player, index: number) => (
                <div key={index} className="flex flex-col gap-2 mt-5 w-full">
                  ...
                  <span className="text-xs text-gray-600"><span className="font-bold text-black italic">Draw :</span>{player.Draw} <span className="font-bold text-black italic">Winner :</span>{player.player1Wins === player.player2Wins ? "Draw" : player.player1Wins > player.player2Wins ? player.player1Name : player.player2Name }</span>
                </div>
              ))}
            </CardContent>
          </CardHeader>
